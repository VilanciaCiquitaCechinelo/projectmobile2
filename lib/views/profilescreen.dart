...
class ProfileScreen extends StatelessWidget {
  ...
  void saveProfileData(BuildContext context) {
    if (nameController.text.length > 30) {
      showSnackBar(context, 'Nama tidak boleh lebih dari 30 karakter.');
      return;
    }
    else if (phoneController.text.isEmpty || !RegExp(r'^\d+$').hasMatch(phoneController.text)) {
      showSnackBar(context, 'Nomor telepon hanya boleh mengandung angka dan tidak boleh kosong.');
      return;
    }
    else if (!RegExp(r'^[a-zA-Z\s]+$').hasMatch(locationController.text)) {
      showSnackBar(context, 'Lokasi hanya boleh mengandung huruf dan spasi.');
      return;
    } else {
